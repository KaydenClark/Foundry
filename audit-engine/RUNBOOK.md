...
The command is read-only toward the target and performs no network or
authentication. It does not fetch, pull, commit, push, merge, clean, prune, or
repair the project. After generic checks pass, it executes the target's explicit
`audit-engine.json` validation contract.

For activity evaluation, Captain performs the authenticated fetch and passes
`--fetch-provenance-json` containing exactly `schemaVersion`, `remoteName`,
`remoteUrl`, `trackedRef`, `sha`, and `fetchedAt`. Audit Engine validates that
contract against local upstream configuration and requires it to be no older
than 48 hours. It never trusts target-writable `FETCH_HEAD`. Any provenance or
upstream-sync failure makes activity unknown.

Named commits are inspected with Git changed-path evidence. A checkpoint that
changes only root `TASKBOARD.md` and/or `Projects/INDEX.md` is generated-only and
does not reset the clock; any other changed path is value-eligible. Seven days
requests owner input and fourteen days emits Ready-to-Archive data. No archive
action is performed.
...
| Activity is `unknown` | Trusted caller provenance is missing/stale/mismatched, upstream sync failed, or no active stable spec owns evidence | Fetch in Captain, pass the exact provenance JSON, then inspect active spec evidence | Rerun read-only evaluation; never infer inactivity from stale evidence |
