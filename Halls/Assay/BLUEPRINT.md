...
**Status:** active
**Source root:** `$INSTANCE_ROOT/Foundry/Halls/Assay`
...
| Foundry artifact judgment | Independent Assay byte/mode, manifest, confidentiality, and immutable-source comparison | Approval is written outside repositories and binds one producer SHA plus tree digest; Assay imports no Forge publisher code |
| Deployment/runtime | Local agent invocation; scheduler is future work | No daemon or scheduled job is claimed yet |
