...
- A read-only, isolated `git ls-remote` may verify an absolute local remote or a
  validated GitHub URL. Target-defined helpers and global credential helpers
  remain disabled. Do not fetch, pull, commit, push, merge, rebase, clean,
  prune, or repair a target.
...
Whole-Foundry artifact approval is a separate read-only Assay seam. It must
recompute evidence independently, import no Forge publisher code, and write its
approval only to a new external path.
