...
and rolling out reusable Foundry and LLM Workbench material. It is not either
product, and the public product repositories are outputs rather than source.

The current Forge source carries the LLM Workbench payload: a reusable
control-doc template for AI-agent projects. It gives a new or existing
repository the files an agent needs before it starts changing code: always-on
operating rules, a compact product map, on-demand capability specs, a hot
execution projection, operational commands, and durable proof.
