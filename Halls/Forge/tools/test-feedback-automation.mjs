...
const escapedWindowsLogPath = ['C:', '\\\\logs'].join('');
const windowsLogPath = ['C:', '\\logs'].join('');
const escapedPipeline = [
  '# Example - Harness Feedback',
  '',
  '| Date | Doc / section | What happened | Impact | Proposed change | Status |',
  '|---|---|---|---|---|---|',
  `| 2026-07-03 | RUNBOOK.md | Command \`node test \\| tee proof.log\` lost its pipe | high | Preserve \`node test \\| tee proof.log\` and \`${escapedWindowsLogPath}\` | new |`
].join('\n');
...
assert.equal(pipelineRows[0].proposedChange, `Preserve node test | tee proof.log and ${windowsLogPath}`);
