...
This always-loaded file owns how agents work in GPT_OS's producer-side Forge
Hall. The Forge develops and evaluates reusable source that will be packaged
into the Foundry and LLM Workbench products; it is neither product. Forge
direction loads from `BLUEPRINT.md` only for architecture work; shared
definitions load from `LEXICON.md` when project language matters; current work
comes from the spec tool and one assigned `SPEC.md`; commands live in
`RUNBOOK.md`.
...
Read anything in this Hall. If a committed secret, credential, or token is
...
`tools/`, `evals/`, `outcomes/`, `benchmarks/`, and this Hall's root
control/docs files.
Do not edit `LICENSE` without an explicit request, `research papers/`, or
anything outside this Hall. Cross-GPT_OS changes require explicit authority
from the GPT_OS root task. Stop if the correct change needs broader scope.

Dogfood boundary:

- `templates/` stays generic, copy-ready, and `[BRACKETED]`.
- Hall controls stay filled, current, and free of template placeholders.
