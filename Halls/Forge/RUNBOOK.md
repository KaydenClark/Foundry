...
**Environment:** GPT_OS producer workspace; private repo `github.com/KaydenClark/GPT_OS`
**Product output:** public repo `github.com/KaydenClark/LLM_Workbench`

This file explains how to operate, verify, and evaluate the producer-side Forge
and its Workbench payload. The Forge is not the LLM Workbench product. Root
S-027 owns current Forge bootstrap and the later whole-Foundry package/publish
pipeline; consolidation is not a prerequisite for the bootstrap checks here.
...
Required accounts/services for producer work:

- GitHub (private producer repo `KaydenClark/GPT_OS`)

There is no environment configuration and no `.env`.

## Install

Nothing to install. Operate from the GPT_OS-owned source:

```bash
cd $INSTANCE_ROOT/Foundry/Halls/Forge
```
...
node tools/test-foundry-source-root.mjs
node tools/test-foundry-publisher.mjs
...
### Foundry Source-Root Contract

`$INSTANCE_ROOT/Foundry/source-root.json` is the machine-readable boundary
between tracked GPT_OS producer source and the whole-Foundry product. The Forge
validator resolves an explicit Git ref to an immutable commit, reads the
contract from that commit, inventories tracked `Foundry/` paths, maps included
files to product-relative paths, and fails closed on installed Modules,
generated Skills, private Wiki/project content, worktrees, secrets,
runtime/build output, Git metadata, or unclassified producer paths.

From the current producer-side Forge:

```bash
node tools/foundry-source-root.mjs validate --ref HEAD
node tools/foundry-source-root.mjs inventory --ref HEAD
node tools/foundry-source-root.mjs inventory --ref HEAD --json
node tools/test-foundry-source-root.mjs
node tools/test-foundry-publisher.mjs
```

`--ref` is mandatory; the command reports the resolved producer SHA. The
inventory is read-only and writes no staging tree or remote. Later
source-recovery tickets widen only explicit allowlisted product paths.

### Whole-Foundry Publisher

Build and verify a deterministic clean product from an explicit immutable
producer ref. The output directory must not already exist:

```bash
node tools/foundry-publisher.mjs dry-run \
  --repo /ABSOLUTE/PRODUCER --ref PRODUCER_SHA --output /ABSOLUTE/ARTIFACT
# Add --json only when the complete file manifest is needed on stdout.
node tools/foundry-publisher.mjs verify \
  --repo /ABSOLUTE/PRODUCER --artifact /ABSOLUTE/ARTIFACT
```

The artifact contains `product/` plus a deterministic
`artifact-manifest.json`. Packaging archives the named SHA, rejects
unclassified/prohibited paths and non-regular files, scans host/credential
patterns, runs the portable product gates, and compares every staged byte and
mode to the immutable producer.

Publication requires the provenance-bearing independent Assay approval whose
three named checks passed and whose `producerSha` and `treeDigest` exactly
match the artifact. Inspect the explicit refspec first:

```bash
node tools/foundry-publisher.mjs plan-publish \
  --artifact /ABSOLUTE/ARTIFACT --approval /ABSOLUTE/APPROVAL.json \
  --target integration --json
```

Only a dedicated clean product checkout on `integration` may be written. The
mutating form requires the literal `--push` flag and always pushes the explicit
`HEAD:integration` refspec:

```bash
node tools/foundry-publisher.mjs publish \
  --repo /ABSOLUTE/PRODUCER --artifact /ABSOLUTE/ARTIFACT \
  --approval /ABSOLUTE/APPROVAL.json --target integration \
  --product-repo /ABSOLUTE/DEDICATED_PRODUCT_CHECKOUT \
  --expected-remote https://github.com/KaydenClark/Foundry.git \
  --github-auth-via-gh --push
```

Never point `--product-repo` at producer source or a shared dirty checkout.
The checkout's configured remote must match `--expected-remote` exactly after
normalization. `--github-auth-via-gh` is explicit and GitHub-HTTPS-only; it
uses the fixed `/opt/homebrew/bin/gh auth git-credential` adapter without
enabling ambient helpers or printing tokens. `main` is mechanically rejected.
...
`Scheduled/workbench-v1-rollout` is tracked by GPT_OS outside this Hall. GPT_OS
owns that scheduler adapter and any persisted automation definition; change it
only from an explicitly authorized GPT_OS root task.
...
| feedback discovery returns no candidate unexpectedly | checkout is a worktree/duplicate, origin is not writable-owner, or fingerprint is already pending/processed | `node tools/feedback-automation.mjs discover --projects-root $INSTANCE_ROOT/Projects` | repair the canonical checkout or record the pending/processed decision; do not broaden discovery |
