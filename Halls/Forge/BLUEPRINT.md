...
**Source root:** `$INSTANCE_ROOT/Foundry/Halls/Forge`
**Producer repository:** private `github.com/KaydenClark/GPT_OS`
**Product output:** public `github.com/KaydenClark/LLM_Workbench`

## Product Map

The Forge is GPT_OS's producer-side Hall for developing, evaluating, releasing,
and rolling out reusable Foundry and LLM Workbench material. It is not the
Foundry product or the LLM Workbench product. This source tree currently carries
the vendor-neutral Workbench payload; exact whole-Foundry source recovery and
publisher mechanics are owned and implemented through root S-027/TK-007 and
TK-008.

The LLM Workbench output is a vendor-neutral, local-first control harness for
AI-assisted software projects. `AGENTS.md` is its always-loaded operating
system; the other surfaces reveal product and execution detail only when it is
relevant.
...
| [S-015 - One-Project Forge Pilot And Version Rollout](specs/S-015-one-project-pilot-and-rollout/SPEC.md) | Define a bounded one-project Forge pilot with exact candidate identity, rejection pinning, and owner-gated version rollout without silently changing other projects. | active |
