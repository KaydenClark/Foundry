...
Add `--json` when you need the structured result.

To gate one specific launch instead of diagnosing all of them:

```bash
node tools/preflight.mjs --spec S-### --ticket TK-### --repo /ABSOLUTE/REPO [--push-to BRANCH]
```
...
|---|---|---|
| 0 | Every check passed | Launch is authorized |
